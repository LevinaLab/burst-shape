...
tau_s = 25#250     # unclear but should be slow
...
#%% plot only flow field for "a"
a_diff = lambda a_old, s_old: -a_old + a_inf(w * s_old * a_old - theta_0) # original
# a_diff = lambda a_old, s_old: s_old * (1 - a_old) * a_old * w - a_old * (1 - s_old) + .01
# a_diff = lambda a_old, s_old: s_old - a_old * (a_old - .1) * (a_old - 1) - .3  # gemini attempt
# a_diff = lambda a_old, s_old: -a_old + a_inf(.17- .8 * (1-s_old) * (1-a_old)) # do (1-x) form
# a_diff = lambda a_old, s_old: -a_old + s_old * a_inf(.5 * s_old * a_old - theta_0) # multiply with s
...
magnitude = np.sqrt(dA**2)
eps = 1e-8
U = dA / (magnitude + eps)
V = np.zeros_like(U)

fig3, ax3 = plt.subplots(figsize=(6,6))
q = ax3.quiver(A, S, U, V, magnitude, cmap='plasma', scale=30, pivot='mid', width=0.005)
cb = fig3.colorbar(q, ax=ax3)
cb.set_label('vector speed (unnormalized)')

# nullcline: contour where dA==0
ax3.contour(A, S, dA, levels=[0], colors='C0', linewidths=2, linestyles='--')

ax3.set_xlabel('a (activity)')
ax3.set_ylabel('e (energy)')
ax3.set_title('Flow field for activity "a"')
ax3.grid(True)
plt.show()