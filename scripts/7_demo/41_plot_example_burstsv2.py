...
for i_cluster in range(1, len(idx_bursts_to_plot) + 1):
    ...
    for idx in idx_bursts_to_plot[i_cluster - 1]:
        ...
        st, gid = np.loadtxt(
            "../data/extracted/%s-%s-%s.spk.txt" % (batch, cult, day)
        ).T
        st = st * 1000
        # fig, ax = plt.subplots()
        ax[i_cluster - 1].plot(
            st, gid, "|", ms=1, color=palette[i_cluster - 1], alpha=0.5
        )
        sc, bins = np.histogram(st, np.arange(0, np.max(st), 50))
        ax[i_cluster - 1].plot(
            bins[1:], 20 * (sc / np.max(sc)) - 30, color=palette[i_cluster - 1]
        )
        ...
        if i_cluster == 7:
            ax[i_cluster - 1].plot(
                [start - 800, (start - 800) + 500], [-10, -9], color="k"
            )
