...
def get_classical_features(df_cultures, df_bursts, dataset):
    ...
    Computes as in van Hugte et al., 2023, but skipping those aspects that we
    don't analyze.
    ...
    """
    assert all(df_cultures["n_bursts"] > 0), (
        "df_cultures must have at least one burst per culture! Before calling this function, remove unnecessary rows with df_cultures = df_cultures[df_cultures['n_bursts'] > 0]"  # noqa: E501
    )
    ...
    if dataset == "wagenaar":
        for index in tqdm(df_cultures.index, desc="Compute classical features"):
            ...
            # MFR
            # MFR was calculated for each well individually by averaging the firing rate of each separate channel by the total number of active channels of the well  # noqa: E501
            ...
            # MBR
            # The mean burst rate (MBR) was calculated by averaging the burst rate across all electrodes.  # noqa: E501
            ...
            # NBR
            # The NB rate (NBR) was derived by dividing all NBs in the recording by the length of the recording in minutes.  # noqa: E501
            ...
            # PRS
            # The percentage of random spikes (PRS) was defined by calculating the percentage of isolated spikes that did not belong to a burst.  # noqa: E501
        ...
        # The IBI and NB IBI (NIBI) were calculated by subtraction of the time stamp at the beginning from the time stamp at the end of each burst or NB  # noqa: E501

        # HFB
        # High frequency bursts (HFBs) within an NB period were detected by decreasing the burst detection ISI to 5 ms, with a maximum IBI of 10 ms, to individually detect each HFB.  # noqa: E501
